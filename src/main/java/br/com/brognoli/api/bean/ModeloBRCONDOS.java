...
public class ModeloBRCONDOS {
	...
	public void lerLinhaDigitavel(List<Linhas> linhas) {
		String codigobarras = "";
		for (int i=0;i<linhas.size();i++) {
			if (linhas.get(i).getLinha().equalsIgnoreCase("237-2")) {
				codigobarras = linhas.get(i-1).getLinha().substring(0, 49);
				i = linhas.size() + 100;
			}else if (linhas.get(i).getLinha().contains("341-7")) {
				codigobarras = linhas.get(i-1).getLinha().substring(0, 49);
				i = linhas.size() + 100;
			} 
		}
