...
public class ModeloDuplique {
	...
	public void lerResumo(List<Linhas> linhas) {
		...
		for (int i=0;i<linhas.size();i++) {
			if ((linhas.get(i).getLinha().contains("Rateio das despesas do mês")) && (!lendo)){
				...
				if (!desc.isEmpty()) {
				int tdesc = desc.length();
				desc = desc.replace(".", "");
				desc = desc.replace("R$", "");
				despesa.setDescricao(desc);
				String valor = linhas.get(i).getLinha().substring(tdesc, linhas.get(i).getLinha().length());
				valor = valor.replace(".", "");
				valor = valor.replace(",", ".");
				valor = valor.replace("", "");
				if (!valor.isEmpty()) {
			...
			if (linhas.size()>(i+1)) {
				if ((linhas.get(i+1).getLinha().contains("Recibo do Pagador"))) {
					i = linhas.size() + 100;
				} 
			}
		}
	...
	public String getDescricao(String linha) {
		...
		for (int i=linha.length();i>0;i--) {
			if (linha.charAt(i-1)!='$') {
				...
				return linha.substring(0, (linha.length()-(contador-2)));
			}
		}
		return "";
	}
	...
	public void lerLinhaDigitavel(List<Linhas> linhas) {
		String codigobarras = "";
		for (int i=0;i<linhas.size();i++) {
			if (linhas.get(i).getLinha().contains("Carteira")) {
				codigobarras = linhas.get(i+1).getLinha();
				i = linhas.size() + 100;
			} 
		}
		if (codigobarras.contains("RCR")) {
			for (int i=0;i<linhas.size();i++) {
	...
	public void lerEndereco(List<Linhas> linhas) {
		String endereco = "";
		for (int i = 0; i < linhas.size(); i++) {
			if (linhas.get(i).getLinha().contains("Recibo do Pagador")) {
