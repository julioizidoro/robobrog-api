...
public class ModeloControll {
	...
	public void lerResumo(List<Linhas> linhas) {
		...
		for (int i=0;i<linhas.size();i++) {
			if ((linhas.get(i).getLinha().contains("Lançamentos do Mês")) && (!lendo)){
				lendo = true;
			} else if (lendo)  {
				...
				if (linhas.size()>(i+1)) {
					if ((linhas.get(i+1).getLinha().contains("TOTAL")) || (linhas.get(i+1).getLinha().contains("Total"))) {
						i = linhas.size() + 100;
					} 
				}
	...
	public void lerLinhaDigitavel(List<Linhas> linhas) {
		String codigobarras = "";
		for (int i=0;i<linhas.size();i++) {
			if (linhas.get(i).getLinha().contains("PAGAVEL")) {
				codigobarras = linhas.get(i-1).getLinha();
				i = linhas.size() + 100;
			} else if (linhas.get(i).getLinha().contains("PAGAR")) {
				codigobarras = linhas.get(i-1).getLinha();
				i = linhas.size() + 100;
			}  else if (linhas.get(i).getLinha().contains("Pagável")) {
				codigobarras = linhas.get(i-1).getLinha();
				i = linhas.size() + 100;
			} 
		}
		codigobarras = codigobarras.replace("BANSICREDI 748-0", "");
		codigobarras = codigobarras.replace("104-0", "");
		codigobarras = codigobarras.replace("UNICRED 136-8", "");
	...
	public void lerEndereco(List<Linhas> linhas) {
		String endereco = "";
		for (int i = 0; i < linhas.size(); i++) {
			if (linhas.get(i).getLinha().equalsIgnoreCase("Destinatário")) {
