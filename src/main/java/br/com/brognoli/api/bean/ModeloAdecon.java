...
public class ModeloAdecon {
	...
	public List<Despesas> listarDespesas(List<Linhas> linhas, String despesa){
		...
		int d=0;
		for (int i=0;i<linhas.size();i++) {
			if (linhas.get(i).getLinha().contains(retirarPontosDescricao(despesa))) {
				d=i+1;
				i=linhas.size()+100;
			}
		}
	...
	public String retirarPontosDescricao(String linha) {
		String desc = "";
		for (int i=0;i<linha.length();i++) {
			if (linha.charAt(i)=='.') {
				i = linha.length() + 100;
			}else {
				desc = linha.substring(0,i);
