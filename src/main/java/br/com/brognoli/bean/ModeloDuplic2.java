...
public class ModeloDuplic2 {
	...
	public void lerResumo(List<Linhas> linhas) {
		...
		for (int i=0;i<linhas.size();i++) {
			if ((linhas.get(i).getLinha().contains("Composição da Cobrança")) && (!lendo)){
				...
				String desc = linhas.get(i).getLinha().substring(7, linhas.get(i).getLinha().length());
				despesa.setDescricao(desc);
				posValor++;
				String valor = linhas.get(posValor).getLinha();
				valor = valor.replace(".", "");
				valor = valor.replace(",", ".");
				valor = valor.replace("", "");
				if (!valor.isEmpty()) {
			...
			if (linhas.size()>(i+1)) {
				if ((linhas.get(i+1).getLinha().contains("CPF/CNPJ"))){ 
					i = linhas.size() + 100;
				} 
			}
		}
	...
	public void lerEndereco(List<Linhas> linhas) {
		String endereco = "";
		for (int i = 0; i < linhas.size(); i++) {
			if (linhas.get(i).getLinha().contains("Pagador")) {
