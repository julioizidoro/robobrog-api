	...
	public void lerResumo(List<Linhas> linhas) {
		...
		for (int i=0;i<linhas.size();i++) {
			if ((linhas.get(i).getLinha().contains("TAXA DE CONDOMÍNIO")) && (!lendo)){
				...
				String valor ="";
				String desc = "";
				if (linhas.get(i).getLinha().contains("GÀS")) {
					valor = getValor(linhas.get(i+2).getLinha());
					desc = linhas.get(i).getLinha();
				} else {
					valor = getValor(linhas.get(i).getLinha());
					desc = linhas.get(i).getLinha().substring(0, linhas.get(i).getLinha().length()-valor.length());
				}
				...
				if (linhas.size()>(i+1)) {
					if ((linhas.get(i+1).getLinha().contains("TOTAL INADIMPLÊNCIA"))){
						i = linhas.size() + 100;
					} 
				}
